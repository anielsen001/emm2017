 ...
 *  Revision Number: $Revision: 1521 $
 *  Last changed by: $Author: awoods $
 *  Last changed on: $Date: 2017-01-24 10:52:41 -0700 (Tue, 24 Jan 2017) $
 ...
 */
{
	...
	int lat_bound[2] = {LAT_BOUND_MIN, LAT_BOUND_MAX};
	int lon_bound[2] = {LON_BOUND_MIN, LON_BOUND_MAX};
...
void MAG_GetDeg(char* Query_String, double* latitude, int bounds[2]) {
    ...
    while(!done)
    {
        ...
        if((value < bounds[0] || (value > bounds[1] && UpBoundOn)) && done == 1) {
			if(UpBoundOn) {
				...
				printf(" Elevations between %d km and %d km are needed for accurate results. \n", bounds[0], bounds[1]);
